...
OldInvoicesWidget::OldInvoicesWidget(int wid, QWidget *parent) :
    QWidget(parent)
{
    width = wid;
    QHBoxLayout *hlay1, *hlay2, *hlay3, *hlay4, *hlay5, *hlay6, *hlay7, *hlay8, *hlay9, *hlay10, *hlay11, *hlay12, *hlay13;
    ...
    QLabel *lable23 = new QLabel("<b>RETAIL INVOICE</b>");
    ...
    hlay13->addWidget(label22);
    hlay13->addWidget(invoiceNo);
    hlay13->addStretch();
...
void OldInvoicesWidget::setData()
{
    clearCustomerData();
    int largest = Commons::getLargestInvoiceNo();
    invoiceNo->setMaximum(largest);
//    invoiceNo->setValue(largest);
    ...
    tableValues.append(TableInvoice::getInvoiceDetails(invoiceNo->text()));

    QString custId = TableInvoice::getCustomerId(invoiceNo->text());
    ...
    setDateEdit(TableInvoice::getDate(invoiceNo->text()));
...
void OldInvoicesWidget::setDate(QDate date)
{
    invoiceDate = date;
}

void OldInvoicesWidget::setInvoiceNo(int inNo)
{
    currentInvoiceNo = QString::number(inNo);
    setData();
}

void OldInvoicesWidget::checkId(QString text)
{
    if(text == "")
    {
        return;
...
void OldInvoicesWidget::checkCode(QString text)
{
    if(text == "")
    {
        return;
...
void OldInvoicesWidget::clearCustomerData()
{
    name->setText("");
    address->setText("");
    phoneNo->setText("");
...
void OldInvoicesWidget::clearItems()
{
...
void OldInvoicesWidget::addAnItem()
{
...
void OldInvoicesWidget::returnItem()
{
    QString id = comId->currentText();
    QString name = commodity->currentText();
    if( id == "" || name == "")
...
void OldInvoicesWidget::removeItem()
{
...
void OldInvoicesWidget::removeLastItem()
{
...
bool OldInvoicesWidget::printInvoiceHtml(int from)
{
    if(tableValues.size() <= 1)
        return false;
    ...
    invoiceData.append(invoiceNo->text());
...
void OldInvoicesWidget::saveInvoice()
{
    if(tableValues.size() <= 1)
        return;
    DatabaseServer *server = new DatabaseServer;
    // Cancel Invoice
    QStringList data = server->getMultiListResult("select CommodityId, Quantity from Invoice where InvoiceNo=" + currentInvoiceNo);
    ...
    TableInvoice::deleteInvoice(currentInvoiceNo);
    ...
    for(int i = 0; i < table->rowCount(); i++)
    {
        ...
        delete server;
        TableInvoice::addNewEntry(currentInvoiceNo, QString::number(customerId), table->getItem(i, 0), table->getItem(i, 1), table->getItem(i, 2), table->getItem(i, 3), table->getItem(i, 4), table->getItem(i, 5), table->getItem(i, 8), invoiceDate);
    }
}

void OldInvoicesWidget::printAndSave()
{
...
void OldInvoicesWidget::saveOnly()
{
    saveInvoice();
    QMessageBox msgBox;
...
void OldInvoicesWidget::totalCorrection(QString total)
{
...
OldInvoicesWidget::~OldInvoicesWidget()
{
}
