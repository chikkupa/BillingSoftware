...
EstimateWidget::EstimateWidget(int wid, QWidget *parent) :
    QWidget(parent)
{
    ...
    QLabel *label22 = new QLabel("<b>ESTIMATE</b>");
...
void EstimateWidget::setData()
{
    clearDealerData();
    Commons::setNextEstimate();
    setHeader(Commons::getHeaderFromTableEstimate());
...
void EstimateWidget::setDate(QDate date)
{
    estimateDate = date;
}

void EstimateWidget::checkId(QString text)
{
    if(text == "")
    {
        return;
...
void EstimateWidget::checkCode(QString text)
{
    if(text == "")
    {
        return;
...
void EstimateWidget::clearDealerData()
{
    name->setText("");
    address->setText("");
    phoneNo->setText("");
...
void EstimateWidget::clearItems()
{
...
void EstimateWidget::removeItem()
{
...
void EstimateWidget::removeLastItem()
{
...
bool EstimateWidget::printEstimateHtml(int from)
{
    if(tableValues.size() <= 1)
        return false;
    ...
    estimateData.append(Commons::getEstimateNo());
...
void EstimateWidget::saveEstimate()
{
    if(tableValues.size() <= 1)
        return;
    DatabaseServer *server = new DatabaseServer;
    QString estimate = server->getSingleResult("select Value from Details where Field='Estimate'");
    // Cancel Estimate
    QStringList data = server->getMultiListResult("select CommodityId, Quantity from Estimate where EstimateNo=" + estimate);
    ...
    TableEstimate::deleteEstimate(estimate);

    int customerId = TableCustomers::addNewCustomer(name->text(), address->text(), phoneNo->text(), mobile->text(), email->text());

    for(int i = 0; i < table->rowCount(); i++)
    {
        TableEstimate::addNewEntry(estimate, QString::number(customerId), table->getItem(i, 0), table->getItem(i, 1), table->getItem(i, 2), table->getItem(i, 3), table->getItem(i, 4), table->getItem(i, 5), table->getItem(i, 8), estimateDate);
    }
}

void EstimateWidget::printAndSave()
{
...
void EstimateWidget::totalCorrection(QString total)
{
