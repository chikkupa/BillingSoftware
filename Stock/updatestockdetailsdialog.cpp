...
UpdateStockDetailsDialog::UpdateStockDetailsDialog(QWidget *parent) :
    QDialog(parent)
{
    status = true;
    setWindowTitle("Edit Stock Details- Dialog");
    resize(500, 350);
    ...
    hlay7->addWidget(label6);
    hlay7->addWidget(cess);
    hlay8->addWidget(label8);
    hlay8->addWidget(codeCombo);
    ...
    connect(codeCombo, SIGNAL(currentIndexChanged(QString)), this, SLOT(checkId(QString)));
...
void UpdateStockDetailsDialog::checkFilled(QString val)
{
...
void UpdateStockDetailsDialog::updateStockDetails()
{
    ids = TableCommodity::getCommodityIds();
    codeCombo->clear();
    values = TableCommodity::getAllCommodities();
    codeCombo->addItems(ids);
    QStringList data = values.at(0).split("\n");
    code->setText(data.at(0));
    name->setText(data.at(1));
    tax->setText(data.at(2));
    purchasePrice->setText(data.at(3));
    value->setText(data.at(4));
    cess->setText(data.at(5));
    stock->setText(data.at(6));
    if(exec())
    {
        TableCommodity::editItem(code->text(), name->text(), tax->text(), purchasePrice->text(), value->text(), cess->text(), stock->text(), codeCombo->currentText());
    }
}

void UpdateStockDetailsDialog::checkId(QString text)
{
    int ind = -1;
