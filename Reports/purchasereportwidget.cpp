...
PurchaseReportWidget::PurchaseReportWidget(int wid, QWidget *parent) :
    QWidget(parent)
{
    width = wid;
    ...
    header->setText(Commons::getHeaderWithAddress() + "<br><center><b>Purchase Report<br></b></center>");
...
void PurchaseReportWidget::setDate1(QDate date)
{
    date1 = date;
    setData();
}

void PurchaseReportWidget::setDate2(QDate date)
{
    date2 = date;
    setData();
}

void PurchaseReportWidget::setData()
{
    QStringList values;
    if(normalView->isChecked())
    {
        DatabaseServer server;
        values = server.getListTableResult(" select  date_format(Date(A.Time),'%d/%m/%Y') as Date, A.InvoiceNo, B.Name as Dealer, sum(A.TaxableValue) as Sales, sum(A.TaxAmount) as Tax, sum(A.Total) as Total from StockInvoice A left join Customers B on A.CustomerId=B.SlNo where A.Time >= '" + date1.toString("yyyy-MM-dd") + "' and A.Time <= '" + date2.toString("yyyy-MM-dd") +"' group by InvoiceNo;");
    }
    else
    {
        QStringList distinctTaxes = TableStockInvoice::getDistinctTaxInBetweenDates(date1, date2);
        DatabaseServer server;
        QString query = "select  date_format(Date(A.Time),'%d/%m/%Y') as Date, A.InvoiceNo, B.Name as Dealer, ";
        ...
        query += "sum(A.TaxableValue) as 'Total Sales', sum(A.TaxAmount) as 'Output Tax', sum(A.Total) as Total from StockInvoice A left join Customers B on A.CustomerId=B.SlNo where A.Time >= '" + date1.toString("yyyy-MM-dd") + "' and A.Time <= '" + date2.toString("yyyy-MM-dd") +"' group by InvoiceNo";
    ...
    setTotal("<font size=4><center><b>Grand Total: " + Commons::getCommaSeperatedValue(total) + "<font size=4><center><b>Total Purchase: " + Commons::getCommaSeperatedValue(sales) );
}

void PurchaseReportWidget::printReport()
{
    QString date = QDate::currentDate().toString("dd-MM-yyyy");
    QString filename = "PurchaseReport.pdf";
    QString content;
...
//        out<<"#tabd{ border: 1px solid #000; border-width: 0 1px 1px 0; }body {background: white;}";
        ...
        out<<"<br><br><center><b>" + Commons::insertNbspInBetween("Purchase Report: ", 1) + Commons::insertNbspInBetween(date1.toString("dd/MM/yyyy"), 1) + Commons::insertNbspInBetween(" to ", 1) + Commons::insertNbspInBetween(date2.toString("dd/MM/yyyy"), 1) + "</b></center><br>";
...
void PurchaseReportWidget::exportToCSV()
{
...
void PurchaseReportWidget::openAsSpreadSheet()
{
    QString filename = "sample.csv";
